...
@authenticate
def get_single_scores_by_user_id(resp, score_id):
    ...
    try:
        score = Score.query.filter_by(
            id=int(score_id), user_id=int(resp['data']['id'])).first()
        if not score:
...
def add_scores(resp):
    ...
    try:
        db.session.add(
            Score(user_id=resp['data']['id'], exercise_id=exercise_id, correct=correct))
        db.session.commit()
