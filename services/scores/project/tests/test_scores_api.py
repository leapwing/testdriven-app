...
class TestScoresService(BaseTestCase):
    ...
    def test_all_scores_by_user_id_no_header(self):
        """Ensure error is throw if 'Authorization' header  is empty."""
        response = self.client.get('/scores/user')
        data = json.loads(response.data.decode())
    ...
    def test_single_score_by_user_id(self):
        """Ensure get single score by user id behaves correctly"""
    ...
    def test_single_score_by_user_id_no_id(self):
        ...
        with self.client:
            response = self.client.get(f'/scores/user/blah',
                                       headers=({'Authorization': 'Bearer test'}))
            data = json.loads(response.data.decode())
    ...
    def test_single_score_incorrect_id(self):
        """Ensure error is throw if an id is not exist."""
        with self.client:
    ...
    def test_single_score_by_user_id_no_header(self):
        """Ensure error is throw if 'Authorization' header  is empty."""
        response = self.client.get('/scores/user/999')
        data = json.loads(response.data.decode())
        self.assertEqual(response.status_code, 403)
        self.assertIn('Provide a valid auth token.', data['message'])
        self.assertIn('error', data['status'])
    ...
    def test_add_score(self):
        """Ensure a new score can be added to the darabase."""
        with self.client:
    ...
    def test_add_score_no_header(self):
        """Ensure error is thrown if the 'Authorization' header is empty."""
        with self.client:
            response = self.client.post(
                '/scores',
                data=json.dumps({
