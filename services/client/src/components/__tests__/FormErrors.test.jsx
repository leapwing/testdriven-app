...
testData.forEach((el) => {
	...
	it(`FormErrors (with ${el.formType} form) renders properly`, () => {
		...
		const liGroup = wrapper.find('li');
		expect(liGroup.length).toBe(Object.keys(el.formRules).length);
		expect(liGroup.get(0).props.children).toBe(el.formRules[0].name);
		expect(liGroup.get(1).props.children).toBe(el.formRules[1].name);
	});
});
